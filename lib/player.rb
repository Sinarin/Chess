...
class Player
  attr_accessor :colour, :side, :alive, :opponent, :king, :game, :dead
  ...
  def check?
    ...
    @opponent.alive.each do |piece|
      ...
      if piece.valid_moves.any? {|position| position.same_values?(@king.current_position)}
        return true
      end
    end
    false
...
=begin
        p piece.current_position if !simulate_move_for_check?(move, piece)
        p move if !simulate_move_for_check?(move, piece)
=end
...
player2 = Player.new('player2', 'white', 'bottom', board, player)
player.opponent = player2
rook1 = Rook.new(board, Position.new(2, 2), player)
rook1.set_piece
rook = Rook.new(board, Position.new(1, 1), player)
rook.set_piece
king = King.new(board, Position.new(7,1), player2)
king.set_piece
pawn = Pawn.new(board, Position.new(8, 2), player2)
pawn.set_piece
player2.alive << pawn
player.alive << rook1
player.alive << rook
